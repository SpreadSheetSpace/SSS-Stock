...
function getMonthYearString(dt) {
    if (language == "it-IT") {
        return ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio',
            'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembre'][dt.getMonth()] +
            ' ' + dt.getFullYear();
    } else {
...
function parseMyDate(date) {
  if (date == "") return new Date('NotADate'); // For Safari
  var a = date.split('/');
  var m, d;
  if (language == "it-IT") {
      m = a[1] - 1;
  ...
  return new Date(a[2], m, d, 0, 0, 0, 0);
}
...
function createCalendar(div, month) {
  ...
  if (language == "it-IT") {
      ...
      daysRow.insertCell(-1).innerHTML = "Gio";
